...
runtime state, Android Presentation routing state, gesture settings, and
AirVision app preferences without gateway endpoints, auth tokens, chat history,
or raw USB serial values. The app does not send ASUS vendor reports until the
Windows app protocol is captured and validated.
