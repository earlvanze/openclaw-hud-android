...
sizes, USB class/subclass/protocol, interface count, USB descriptor version when Android exposes it, per-feature firmware-apply readiness for Windows-style AirVision
...
candidate counts, selected display identity, Windows-only cursor/3DoF and Unity
mirror-window capability status, Windows-only firmware-update workflow status,
gesture settings, and AirVision app preferences without gateway endpoints, auth
tokens, chat history, or raw USB serial values. Settings ->
