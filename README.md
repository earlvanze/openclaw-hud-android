...
- [x] Recent OpenClaw sessions can be selected directly on the external HUD; session and agent context controls refresh on open and lock during active runs and execution approvals
...
- Switch recent OpenClaw sessions from the history control and the active agent from the account control beside the HUD session line; both controls refresh on open and lock while a run or execution approval is active.
...
Generic controls live first in Settings -> External Display HUD. Optional AirVision integration tools are collapsed when no matching USB hardware is connected:
...
| IPD | Stored as a calibration value, defaulting to 67 mm. Normal Android adjustment is clamped to the ASUS documented 53.5-74.5 mm range, represented as 54-74 mm slider stops. Adjustment is locked while Light Load Mode is enabled, matching the ASUS app. Firmware-level apply still needs the ASUS HID protocol. |
...
state and step sizes, shortcut-menu parity for ASUS brightness/volume/distance
behavior, a Windows gesture catalog covering ASUS-documented
...
Unity mirror-window capability status, structured Android Cast/Display/Samsung DeX
mirror fallback actions and limitations, a structured Windows app apply matrix that lists each ASUS Windows
target alongside the Android effect, live-M1 proof requirement, firmware gate,
Windows-only state, row-level observed Windows surfaces/keys/defaults/capture
implications, Cyber-observed ASUS AirVision 1.0.7.1 build/settings-key
...
status, gesture settings, structured caption/translation mode status, structured support/legal/registration metadata, and AirVision app preferences without gateway
...
protocol-ready/blocked feature counts, protocol-ready, needs-validation, pending,
