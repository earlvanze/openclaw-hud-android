...
function render(features) {
  const lines = [
    ...
    `- Summary: firmware writes: read-only; 0/${features.length} validated captures, 0 protocol-ready, ${features.length} blocked`,
    "- Firmware writes enabled: no",
    `- Validated captures: 0/${features.length}`,
    `- Protocol-ready captures: 0/${features.length}`,
    `- Blocked features: ${features.length}`,
    "- Protocol-ready feature labels: none",
    `- Blocked feature labels: ${features.map((feature) => feature.label).join(", ")}`,
    ...
    `- Summary: firmware apply preview: 0 protocol-ready, ${features.length} blocked, writes disabled`,
    `- Protocol-ready command count: 0/${features.length}`,
    `- Blocked feature labels: ${features.map((feature) => feature.label).join(", ")}`,
    "- Commands: none",
    "",
