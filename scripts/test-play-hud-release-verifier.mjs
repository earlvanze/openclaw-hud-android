...
const validReadme = [
  ...
  "Diagnostics exports include Android demo/offline reviewer experience state, profile backup/restore readiness, all saved AirVision profile values, runtime profile metadata, structured M1 brightness-key/media-key mapping evidence with Android consumption state and step sizes, Windows gesture catalog with instant transparent and shortcut-menu hold/slide behavior, Cyber-observed ASUS AirVision 1.0.7.1 build/settings-key evidence for VirtualSpaceDistance, SoftwareIPD, DisplaySplendidMode, EyeCareLevel, PreventMotionBlur, IsEcoMode, CenterCursorHotkey, and DistanceHotkey, and companion parity-state counts for offline-reviewable, M1-optional, firmware-gated, and Windows-only features.",
  ...
  "Windows App Handoff exports ASUS Windows app apply steps, ASUS AirVision 1.0.7.1 build/settings-key evidence, VirtualSpaceDistance, SoftwareIPD, DisplaySplendidMode, DistanceHotkey, structured M1 hardware-key mapping, brightness/media key consumption state, Windows gesture catalog entries, and privacy reminders while omitting raw USB serial values.",
