...
class AirVisionFirmwareCaptureResultsTest {
    ...
    @Test
    fun summarize_reportsCapturedFeaturesThatNeedValidation() {
        val captured =
            pendingResultsJson(
                overridesByRawKey =
                    mapOf(
                        "brightness" to
                            ...
                            "writePayloadSummary": "brightness byte changed; sanitized",
                            "readbackReportId": "0x85",
                            "readbackEndpoint": "in if=1 interrupt addr=0x81 max=32 int=4",
                            "readbackPayloadSummary": "readback observed; sanitized",
                            "checksumFramingNotes": "checksum still under review",
                            "visibleStateConfirmed": true,
                            "androidEnablementDecision": "blocked",
                            "blockerReason": "Captured but not independently validated."
                            """.trimIndent(),
                    ),
            )

        val summary = AirVisionFirmwareCaptureResultFiles.summarize(captured)
