...
class HudGestureMotionTrackerTest {
    ...
    fun verticalMovementBecomesSwipeAndIncludesInitialDistance() {
        ...
        assertEquals(HudGestureMotionResult.Swipe, tracker.finish(consumed = false))
    }

    @Test
    fun horizontalOrConsumedMovementIsIgnored() {
        val horizontal = HudGestureMotionTracker(touchSlop = 10f)
        assertNull(horizontal.add(Offset(12f, 3f), consumed = false))
        assertEquals(HudGestureMotionResult.Ignore, horizontal.finish(consumed = false))
